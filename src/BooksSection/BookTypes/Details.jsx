...
const Details = () => {
    ...
    const { id } = useParams()
    const { data: detailsBooks = [] } = useQuery({
        queryKey: ['detailsBooks'],
        queryFn: async () => {
    ...
    return (
        <div className="hero min-h-screen" style={{ backgroundImage: 'url(https://i.ibb.co/ns12sKV/Books-featuring-the-book-reading-and-study.jpg)' }}>
            ...
            <div className=" glass bg-[#90b2ddff]  shadow-2xl my-36 lg:w-[1000px] md:w-[650px] w-[280px]">
                ...
                <div className="flex flex-col lg:flex-row gap-10 p-10 items-center">
                    ...
                    <div className=" lg:w-3/4 text-[#783d19ff]">
                        ...
                        <p className="text-xl font-bold flex">Ratings:<span className=" text-xl"> <Rating
                   style={{ maxWidth: 180, width: '70%',  }} value={book.ratings} readOnly /></span></p>
                        <p className="text-xl font-bold pb-4">Details:{book?.description?.slice(0,200)}... </p>

                        <button onClick={() => setOpenModalAgain(true)} className=" btn glass text-lg mr-4 bg-[#783d19ff] text-[#faf8ea]">Read</button>
                        ...
                        <div onClick={() => setOpenModalAgain(false)} className={`fixed flex justify-center items-center z-[100] ${openModalAgain ? 'visible opacity-1' : 'invisible opacity-0'} inset-0 w-full h-full backdrop-blur-sm text-[#783d19ff] duration-100`}>
                            <div onClick={(e_) => e_.stopPropagation()} className={`absolute w-full lg:w-[500px] bg-[#faf8ea] text-[#783d19ff] drop-shadow-2xl rounded-lg ${openModalAgain ? 'opacity-1 duration-300 translate-y-0' : '-translate-y-20 opacity-0 duration-150'}`}>
                                <div className="p-12">
